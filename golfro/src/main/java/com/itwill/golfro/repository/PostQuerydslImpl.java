...
public class PostQuerydslImpl extends QuerydslRepositorySupport implements PostQuerydsl {
	...
	public Tuple selectByPostId(Long id) {
		...
		// 서브쿼리: 댓글 존재 여부 확인
		JPQLQuery<?> existsSubquery = JPAExpressions
				.selectOne()
				.from(comment)
	...
	public List<Post> selectTop5ByF001() {
	    ...
	    JPQLQuery<Post> query = from(post)
	            .where(post.category.id.eq("F001"))
	...
	public List<Post> selectTop5ByF002() {
	    ...
	    JPQLQuery<Post> query = from(post)
	            .where(post.category.id.eq("F002"))
	            .orderBy(post.likes.desc())
	            .limit(5);
	...
	public Page<Post> selectByCategoryAndKeyword(CommPostSearchDto dto, Pageable pageable) {
	    ...
	    if (dto.getSearchCategory() != null
	        ...
	        switch (dto.getSearchCategory()) {
	            ...
	            case "tc":
	                whereClause.and(post.title.likeIgnoreCase(searchKeyword)
	                        .or(post.content.likeIgnoreCase(searchKeyword)));
	                break;
	            case "n":
	...
	public List<Post> selectOrderByTeeoffDesc() {
        ...
        JPQLQuery<Post> query = from(post)
				.where(post.category.id.eq("F003"))
				.orderBy(post.teeoff.desc());
	...
	public List<Tuple> selectReadAll() {
	    ...
	    List<Tuple> result = queryFactory
	            ...
	            .leftJoin(comment).on(comment.post.id.eq(post.id)
	                .and(comment.selection.eq(1)))
	            .where(post.category.id.eq("P001"))
	            .orderBy(post.id.asc())
	            .fetch();
	...
	private BooleanBuilder buildSearchConditions(MainPostSearchDto dto, String searchKeyword) {
	    ...
	    if (dto.getSearchCategory().equals("searchClubs")) {
	        ...
	        if (dto.getClubSelect().equals("WD")) builder.and(post.club.id.eq("WD"));
	        if (dto.getClubSelect().equals("DR")) builder.and(post.club.id.eq("DR"));
	        if (dto.getClubSelect().equals("IR")) builder.and(post.club.id.eq("IR"));
	    }
	...
	private BooleanBuilder buildSearchConditions(MyPostSearchDto dto, String searchKeyword) {
	    ...
	    if ("searchClubs".equals(dto.getSearchCategory())) {
	        switch (dto.getClubSelect()) {
	            case "WG": builder.and(post.club.id.eq("WG")); break;
	            case "PT": builder.and(post.club.id.eq("PT")); break;
	            case "UT": builder.and(post.club.id.eq("UT")); break;
	            case "WD": builder.and(post.club.id.eq("WD")); break;
	            case "DR": builder.and(post.club.id.eq("DR")); break;
	            case "IR": builder.and(post.club.id.eq("IR")); break;
	        }
	    }
	...
	public Page<Tuple> getPostList(Pageable pageable) {
	    ...
	    JPAQuery<Tuple> query = queryFactory
	            ...
	            .join(club).on(post.club.id.eq(club.id))
	            .join(user).on(post.user.userid.eq(user.userid))
	            .where(post.category.id.in("P001"))
	            .orderBy(post.id.desc());
	...
	public Page<Post> search(MyPostListSearchDto dto, Pageable pageable) {
	    ...
	    // 공통 조건
	    builder.and(post.category.id.ne("P001"))
	           .and(post.user.userid.eq(dto.getUserid()));
	...
	public Page<Post> selectPagedPosts(String userid, Pageable pageable) {
	    ...
	    JPAQuery<Post> query = queryFactory
	        .selectFrom(post)
	        .join(user).on(post.user.userid.eq(user.userid))
	        .where(post.category.id.ne("P001")
	...
	public long selectTotalPostCount(String[] category) {
	    ...
	    JPQLQuery<Post> query = from(post)
	            .where(post.category.id.in(category));
